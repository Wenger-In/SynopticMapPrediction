...
data0 = np.zeros((len(data_mat)-2,1))
lm = 1
l_cor = data_mat[0][lm]
m_cor = data_mat[1][lm]
...
future_predict_ulim = future_predict * (1 + model_loss)
future_predict_llim = future_predict * (1 - model_loss)
...
plt.plot(future_t+len(data0_t),future_predict_ulim, label='future predict upper limit')
plt.plot(future_t+len(data0_t),future_predict_llim, label='future predict lower limit')
plt.legend()
