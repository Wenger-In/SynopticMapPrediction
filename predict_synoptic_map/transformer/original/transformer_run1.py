...
print("x_array shape: {}".format(x_train.shape))
x_valid = []
y_valid = []

for i in tqdm(range(time_stamp, len(new_valid_df)-288, 288)):
    x_valid.append(new_valid_df[i - time_stamp:i])
    y_valid.append(new_valid_df.iloc[i:i+288, 2])

x_valid, y_valid = np.array(x_valid), np.array(y_valid)
print("x_array shape: {}".format(x_valid.shape))
# 定义迭代器
...
train_data = DataLoader(dataset=train_set, batch_size=2048, shuffle=True)
test_data = DataLoader(dataset=valid_set, batch_size=2048, shuffle=False)
...
#np.savetxt( "/22_4day_3hour.txt" , y_)
torch.save(net.state_dict(), "checkpoint.pth.tar")

#de_normalized_df = pd.read_csv("JK_L_result.csv", encoding='utf8', header=None)    #读取数据集
#de_normalized_df.apply(lambda x: (x * (val_max - val_min) + val_min))  # 对每一列数据归一化
#de_normalized_df.to_csv('JK_L_result.csv', encoding='utf8', index=False)
